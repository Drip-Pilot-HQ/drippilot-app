...
const KNOWLEDGE_BASE_KEYS = {
  ...
  lists: () => [...KNOWLEDGE_BASE_KEYS.all, "list"] as const,
  entry: (id: string) => [...KNOWLEDGE_BASE_KEYS.all, "entry", id] as const,
};

export function useKbEntriesQuery() {
  return useQuery({
    queryKey: KNOWLEDGE_BASE_KEYS.lists(),
    queryFn: async () => {
      const response = await apiClient.get<KbEntry[]>("/ai/knowledge-base");
...
export function useCreateKbEntryMutation() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: KNOWLEDGE_BASE_KEYS.lists() });
